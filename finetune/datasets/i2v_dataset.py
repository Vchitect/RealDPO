...
class BaseI2VDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        encoded_video_path = video_latent_dir / (image.stem + ".safetensors")
...
class RealDPOI2VDataset(Dataset):
    """
    Base dataset class for RealDPO Image-to-Video (I2V) training.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        data_root = Path(data_root)
        self.negative_sample = Path(negative_samples_root)
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        while True:
            try:
                ...
                lose_video = self.negative_sample / video.stem
                image = self.images[index]
                ...
                prompt_hash = str(hashlib.sha256(prompt.encode()).hexdigest())
                prompt_embedding_path = prompt_embeddings_dir / (prompt_hash + ".safetensors")
                encoded_video_path = video_latent_dir / (image_stem + ".safetensors")
                ...
                # load lose video
                if not (lose_video_latent_dir / image_stem).exists():
                    (lose_video_latent_dir / image_stem).mkdir(parents=True, exist_ok=True)

                all_lose_encoded_videos = []
                for k in range(self.sample_num):
                    lose_id = k                    
                    encoded_lose_video_path =  lose_video_latent_dir / (image_stem / Path(f"{lose_id}.safetensors"))
                    if encoded_lose_video_path.exists():
                        ...
                        encoded_lose_video_path =  lose_video_latent_dir / (image_stem / Path(f"{k}.safetensors"))
                    ...
                    all_lose_encoded_videos += [encoded_lose_video]
                ...
                return {
                    ...
                    "all_encoded_lose_videos": torch.stack(all_lose_encoded_videos),
                    "video_metadata": {
                        "num_frames": encoded_video.shape[1],
                        "height": encoded_video.shape[2],
                        "width": encoded_video.shape[3],
...
class RealDPOI2VDatasetWithResize(RealDPOI2VDataset):
    """
    A dataset class for image-to-video generation that resizes inputs to fixed dimensions.
